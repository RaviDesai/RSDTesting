...
public class APIConsumersClient {
    public static var sharedClient = APIConsumersClient()
    ...
    public func authenticateUsingFedAuth(userName: String, password: String, callback: ((NSError?) ->())) {
            ...
            if let params = loginResponse?.token {
                ...
                call.executeRespondWithObject({ (stringResult, error) -> () in
                    if (stringResult == .Some("success")) {
                        let endpointUrl = URLAndParameters(url:"PostLogin/PostLoginChecks")
